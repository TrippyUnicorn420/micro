...
main = do
    ...
    let listOfLines = LOL (TELine "" 0 0 0 0 0:[]) 0
    let currentLine = head (lolLines listOfLines)
        in do
            printEditorView listOfLines currentLine
            editorView <- appendToLine listOfLines (pure currentLine)
            printEditorView listOfLines editorView
    putStrLn ("\n" ++ "Thank you for using Micro.")

-- Prints the current contents of the TELine that user should see (strings only)
printEditorView :: LOL -> TELine -> IO()
printEditorView lol editorView = do
    putStr("\n")
    let view = (formatTELine lol editorView)
    printf("\ESC[2J\ESC[H\ESC[0;0f\r")
    putStrLn (view)
...
formatTELine :: LOL -> TELine -> String
formatTELine lol editorView = textContents (formatting lol (TextFormat "" 0 "" 0 (contents editorView) 0 (cursorPos editorView) True (leftBrackPos editorView) True (rightBrackPos editorView) True))
...
formatting :: LOL -> TextFormat -> TextFormat
formatting lol te = do
    ...
    -- Printing cursor
        ...
        if (toFormat /= "") then do
            ...
            if (newChar == " ") then
                if (fLength + newWLength >= 120) then do
                    let currLine = textContents (TextFormat "" 0 (formatted ++ word ++ "\n") 0 (tail toFormat) (totalParsed + 1) cursor isNotPrinted lb lbNotPrinted rb rbNotPrinted)
                    let currLol = TELine currLine 0 0 (totalTELineChars 0 formatted) 0 0:(lolLines lol)
                    formatting (LOL currLol 1) (TextFormat "" 0 ("\n") 0 (tail toFormat) (totalParsed + 1) cursor isNotPrinted lb lbNotPrinted rb rbNotPrinted)
                else
                    formatting lol (TextFormat "" 0 (formatted ++ word ++ newChar) (fLength + newWLength) (tail toFormat) (totalParsed + 1) cursor isNotPrinted lb lbNotPrinted rb rbNotPrinted)
            else if (newChar == "\n") then do
                let currLine = textContents (TextFormat "" 0 (formatted ++ word ++ "\n") 0 (tail toFormat) (totalParsed + 1) cursor isNotPrinted lb lbNotPrinted rb rbNotPrinted)
                let currLol = TELine currLine 0 0 (totalTELineChars 0 formatted) 0 0:(lolLines lol)
                formatting lol (TextFormat "" 0 (newChar) 0 (tail toFormat) (totalParsed + 1) cursor isNotPrinted lb lbNotPrinted rb rbNotPrinted)
            else if (newWLength == 121) then do
                ...
                let currLine = textContents (TextFormat "" 0 (formatted ++ word ++ "\n") 0 (tail toFormat) (totalParsed + 1) cursor isNotPrinted lb lbNotPrinted rb rbNotPrinted)
                let currLol = TELine currLine 0 0 (totalTELineChars 0 formatted) 0 0:(lolLines lol)
                formatting lol (TextFormat newChar 1 ("\n") 1 (tail toFormat) (totalParsed + 1) cursor isNotPrinted lb lbNotPrinted rb rbNotPrinted)
            else if (fLength + newWLength > 120) then do
                let currLine = textContents (TextFormat "" 0 (formatted ++ word ++ "\n") 0 (tail toFormat) (totalParsed + 1) cursor isNotPrinted lb lbNotPrinted rb rbNotPrinted)
                let currLol = TELine currLine 0 0 (totalTELineChars 0 formatted) 0 0:(lolLines lol)
                formatting lol (TextFormat (word ++ newChar) newWLength (formatted ++ "\n") 0 (tail toFormat) (totalParsed + 1) cursor isNotPrinted lb lbNotPrinted rb rbNotPrinted)
            else
                -- add new character to word, keep going
                formatting lol (TextFormat (word ++ newChar) newWLength formatted fLength (tail toFormat) (totalParsed + 1) cursor isNotPrinted lb lbNotPrinted rb rbNotPrinted)
        else
...
appendToLine :: LOL -> IO TELine -> IO TELine
appendToLine lol line =
    line
    >>= \line ->
        getnewInput
        >>= \nextchar ->
            if (nextchar == '\^X')
                then do
                    putStrLn (contents (head (lolLines lol)))
                    pure line
                ...
                do
                    newline <- load
                    printEditorView lol newline
                    appendToLine lol (pure newline)
            else
                if (nextchar == '\n') then
                    -- If text is highlighted, replace it with this character
                    if (leftBrackPos line /= rightBrackPos line) then do
                        printLine lol (TELine (replaceHighlight nextchar (contents line) (leftBrackPos line) (rightBrackPos line) (charTotal line)) ((leftBrackPos line)+1) ((charTotal line) - ((rightBrackPos line) - (leftBrackPos line)) + 1) ((leftBrackPos line)+1) ((leftBrackPos line)+1) ((leftBrackPos line)+1))
                    else do
                        printLine lol (TELine (addChar nextchar (contents line) (cursorPos line) (charTotal line)) ((cursorPos line) + 1) ((charTotal line) + 1) ((cursorPos line) + 1) ((cursorPos line) + 1) ((cursorPos line) + 1))
                        --checkingPrintFormat(newline)
                else if (nextchar == '\DEL') then -- for backspace
                    -- If text is highlighted, delete the highlighted section
                    if (leftBrackPos line /= rightBrackPos line) then do
                        printLine lol (TELine (deleteHighlight (contents line) (leftBrackPos line) (rightBrackPos line) (charTotal line)) (leftBrackPos line) ((charTotal line) - ((rightBrackPos line) - (leftBrackPos line))) (leftBrackPos line) (leftBrackPos line) (leftBrackPos line))
                    else if ((cursorPos line) == 0) then do
                        printLine lol line
                    else do
                        printLine lol (TELine (removeCharLeft (contents line) (cursorPos line) (charTotal line)) (checkCursorDelete (cursorPos line)) ((charTotal line) - 1) (checkCursorDelete (cursorPos line)) (checkCursorDelete (cursorPos line)) (checkCursorDelete (cursorPos line)))
                                                ...
                                                >>= \squigglechar ->
                                                    ...
                                                    if (squigglechar == '~') then
                                                        ...
                                                        -- If something after cursor, delete first char after the cursor
                                                            ...
                                                            printLine lol (TELine (removeCharRight (contents line) (cursorPos line) (charTotal line)) (cursorPos line) ((charTotal line) - 1) (cursorPos line) (cursorPos line) (cursorPos line))
                                                    -- Weird keypress - don't do anything
                                                    else
                                                        do
                                                            printLine lol (TELine (addChar nextchar (contents line) (cursorPos line) (charTotal line)) ((cursorPos line) + 1) ((charTotal line) + 1) (cursorPos line) (cursorPos line) (cursorPos line))
                                                                            ...
                                                                            if (shiftarrow == 'C') then
                                                                                do
                                                                                    ...
                                                                                    -- If highlight is getting larger ([]| to [.....]|)
                                                                                        ...
                                                                                        printLine lol (TELine (contents line) ((cursorPos line) + 1) (charTotal line) ((leftBrackPos line)) ((rightBrackPos line) + 1) (pivotHighlight line))
                                                                                    else if ((pivotHighlight line) < (rightBrackPos line)) then do
                                                                                        printLine lol (TELine (contents line) ((cursorPos line) + 1) (charTotal line) ((leftBrackPos line)) ((rightBrackPos line) + 1) (pivotHighlight line))
                                                                                    -- If highlight is getting smaller ([|....] to [|])
                                                                                    else do
                                                                                        printLine lol (TELine (contents line) ((cursorPos line) + 1) (charTotal line) ((leftBrackPos line + 1)) (rightBrackPos line) (pivotHighlight line))
                                                                                ...
                                                                                -- Else, find the new cursor position
                                                                                    ...
                                                                                    if (pivot == lb && pivot == rb) then do
                                                                                        printLine lol (TELine (contents line) (newCursorPos) (charTotal line) (newCursorPos) (rb) (pivot))
                                                                                    ...
                                                                                    -- If we are reducing the highlight section (get smaller) but highlight section is not yet fully removed
                                                                                        ...
                                                                                        printLine lol (TELine (contents line) (newCursorPos) (charTotal line) (lb) (newCursorPos) (pivot))
                                                                                    -- Else we are highlighting from one direction to another (the brackets have to change)
                                                                                    else do
                                                                                        printLine lol (TELine (contents line) (newCursorPos) (charTotal line) (newCursorPos) (pivot) (pivot))
                                                                                ...
                                                                                -- Else move down a line
                                                                                    ...
                                                                                    -- If pass the pivot, from left, to right, getting slightly bigger on right
                                                                                        ...
                                                                                        printLine lol (TELine (contents line) (newCursorPos) (charTotal line) (pivot) (newCursorPos) (pivot))
                                                                            else
                                                                                printLine lol line
                                                                else
                                                                    printLine lol line
                                        ...
                                        -- IF up arrow is pressed
                                            ...
                                            do
                                                ...
                                                -- Else move down a line
                                                    ...
                                                    printLine lol (TELine (contents line) (newCursorPos) (charTotal line) (newCursorPos) (newCursorPos) (newCursorPos))
                                        else if (numberchar == 'D') then
                                            if (cursorPos line == 0) then do
                                                printLine lol line
                                            else do
                                                printLine lol (TELine (contents line) ((cursorPos line) - 1) (charTotal line) ((cursorPos line) - 1) ((cursorPos line) - 1) ((cursorPos line) - 1))
...
printLine :: LOL -> TELine -> IO TELine
printLine lol line = do
    newline <- pure line
    printEditorView lol newline
    appendToLine lol (pure newline)
...
totalTELineChars :: Int -> String -> Int
