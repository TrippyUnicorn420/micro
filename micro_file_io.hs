...
formatting te = do
    ...
    if (toFormat /= "") then do
        -- Get next character
        let newChar = Prelude.take 1 toFormat
        let newWLength = wLength + 1
        -- If a space, it means the word is finished
        if (newChar == " ") then
            if (fLength + newWLength > 121) then
                formatting (TextFormat "" 0 (formatted ++ word ++ "\n") 0 (Prelude.tail toFormat))
            else
                formatting (TextFormat "" 0 (formatted ++ word ++ newChar) (fLength + newWLength) (Prelude.tail toFormat))
            ...
            formatting (TextFormat (word ++ newChar) newWLength formatted fLength (Prelude.tail toFormat))
    else
